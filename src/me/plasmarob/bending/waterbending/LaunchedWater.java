 ...
 *
 */
public class LaunchedWater
{
	public static ConcurrentHashMap<Integer, LaunchedWater> instances = new ConcurrentHashMap<Integer, LaunchedWater>();
	...
	public LaunchedWater(Player player, Location from, Location to)
	{
		this.player = player;
		counter++;
		id = counter;
	...
	public LaunchedWater(Player player)
	{
		this.player = player;
		counter++;
		id = counter;
		...
		throwPath.setY(throwPath.getY()+0.3);
		throwPath = throwPath.normalize().multiply(0.6);
	...
	public void progress()
	{
		for (int i = 0; i < 2; i++)
		{
			...
			if (LaunchedWater.blockHeld(tmp))
			{
				if (Tools.isWater(prev.getType()))
					prev.setType(Material.AIR);
				instances.remove(id);
	...
	{
		for (LaunchedWater lw : instances.values())
		{
